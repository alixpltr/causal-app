...
# causal-learn (for nice rendering)
...
LAMBDA1   = 0.0    # L1 penalty inside NOTEARS
EPS       = 1e-3   # binarize each run: |W| < EPS -> 0
EDGE_THR  = 0.50   # include pair in consensus if skeleton prob ≥ this
DIR_THR   = 0.6   # orient if conditional same-dir prob ≥ this (and ≥ opposite)
RNG_SEED  = 12345

# ---------- helpers ----------
def clean_df_no_missing(df: pd.DataFrame) -> pd.DataFrame:
    """Numeric-only; ±inf→NaN; drop NaN rows (NOTEARS needs complete data)."""
...
def main():
    ...
    X_full = StandardScaler().fit_transform(df.to_numpy(dtype=float))
    n, p = X_full.shape
    print(f"[NOTEARS-BOOT] Data {X_full.shape} | n_boot={args.n_boot} | eps={EPS}, lambda1={LAMBDA1}")
    ...
    for b in range(args.n_boot):
        ...
        try:
            ...
            print(f"[NOTEARS-BOOT] replicate {b}: FAILED -> {e}")
            continue
    ...
    if B_eff == 0:
        raise RuntimeError("All NOTEARS bootstraps failed; no consensus computed.")
    ...
    # ---------- save consensus PNG ----------
    png_consensus = outdir / "notears_boot_consensus.png"
    if not try_graphviz_png(consensus, labels, png_consensus):
        draw_networkx_fallback(consensus, labels, "NOTEARS bootstrap consensus", png_consensus)
    print(f"[NOTEARS-BOOT] Saved: {png_consensus.name}")
    ...
    try:
        ...
        png_annot = outdir / "notears_boot_consensus_annotated.png"
        pyd.write_png(str(png_annot))
        print(f"[NOTEARS-BOOT] Saved: {png_annot.name}")
    except Exception as e:
        print(f"[NOTEARS-BOOT] Annotation failed: {e}")

    # optional: save raw arrays
    np.save(outdir / "notears_boot_skeleton_prob.npy", skel_prob)
    np.save(outdir / "notears_boot_dir_prob.npy", same_dir_cond)

    print(f"[NOTEARS-BOOT] Bootstraps used: {B_eff}/{args.n_boot}")
    print(f"[NOTEARS-BOOT] Outputs in: {outdir}")
