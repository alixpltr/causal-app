# missdag_boot_test.py — hard-coded paths, n_boot=2
...
# MissDAG + MCEM init/learner (your files must be importable)
...
DATA_PATH  = r"C:\Users\Antoi\Downloads\Projetcausal\Projetcausal\alix\data\signature_cleanedo.csv"
OUTPUT_DIR = r"C:\Users\Antoi\Downloads\Projetcausal\Projetcausal\DAGS"
N_BOOT     = 2

WEIGHT_THR = 0.30   # binarize each run: edge i->j present if |B[i,j]| > WEIGHT_THR
EDGE_THR   = 0.50   # include in consensus if skeleton prob ≥ EDGE_THR
...
def clean_df_keep_missing(df: pd.DataFrame) -> pd.DataFrame:
    """Numeric columns only; convert ±inf→NaN; DO NOT drop NaNs (MissDAG handles missing)."""
...
def try_graphviz_png(GG: GeneralGraph, labels, out_png: Path) -> bool:
    try:
...
def draw_networkx_fallback(consensus: GeneralGraph, labels, title, out_png: Path):
    dir_list, und_list = [], []
...
if __name__ == "__main__":
    ...
    n, p = X_full.shape
    print(f"[MISSDAG-BOOT] Data {X_full.shape} | n_boot={N_BOOT} | weight_thr={WEIGHT_THR}")
    # print("Labels:", labels)  # uncomment if you want to see the variable names
    ...
    for b in range(N_BOOT):
        ...
        try:
            dag_init = Notears_MLP_MCEM_INIT(lambda1=0.01)
            dag_mcem = Notears_MLP_MCEM(lambda1=0.01)
    ...
    print(f"[MISSDAG-BOOT] Saved: {png_consensus}")
    ...
    try:
        ...
        print(f"[MISSDAG-BOOT] Saved: {png_annot}")
    except Exception as e:
        print(f"[MISSDAG-BOOT] Annotation failed: {e}")

    print(f"[MISSDAG-BOOT] Bootstraps used: {B_eff}/{N_BOOT}")
