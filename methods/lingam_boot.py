...
WEIGHT_THR = 0.10   # per-boot binarization: edge exists if |B[i,j]| > THR  (j -> i in LiNGAM)
...
def clean_df_no_missing(df: pd.DataFrame) -> pd.DataFrame:
    """Numeric-only; convert ±inf→NaN; drop NaN rows (DirectLiNGAM needs complete data)."""
...
def main():
    ...
    X = df.to_numpy()
    n, p = X.shape
    print(f"[LINGAM-BOOT] Data {X.shape} | n_boot={args.n_boot} | thr={WEIGHT_THR}")
    ...
    for b in range(args.n_boot):
        ...
        try:
            # DirectLiNGAM: adjacency_matrix_[i,j] is weight for edge j -> i
            model = DirectLiNGAM(random_state=RNG_SEED + b)
            model.fit(Xb)
            B = model.adjacency_matrix_

            # binarize and orient to our convention A_dir[i,j] = 1 means i->j
            A_bin = (np.abs(B) > WEIGHT_THR).astype(int)
            np.fill_diagonal(A_bin, 0)
            A_dir = A_bin.T  # transpose: B[i,j] => j->i, so A_dir[i,j] = A_bin[j,i]
        except Exception as e:
            print(f"[LINGAM-BOOT] replicate {b}: FAILED -> {e}")
            continue
    ...
    if B_eff == 0:
        raise RuntimeError("All LiNGAM bootstraps failed; no consensus computed.")
    ...
    # ---------- save consensus PNG ----------
    png_consensus = outdir / "lingam_boot_consensus.png"
    if not try_graphviz_png(consensus, labels, png_consensus):
        draw_networkx_fallback(consensus, labels, "DirectLiNGAM bootstrap consensus", png_consensus)
    print(f"[LINGAM-BOOT] Saved: {png_consensus.name}")
    ...
    try:
        ...
        png_annot = outdir / "lingam_boot_consensus_annotated.png"
        pyd.write_png(str(png_annot))
        print(f"[LINGAM-BOOT] Saved: {png_annot.name}")
    except Exception as e:
        print(f"[LINGAM-BOOT] Annotation failed: {e}")

    # optional: save raw arrays for debugging
    np.save(outdir / "lingam_boot_skeleton_prob.npy", skel_prob)
    np.save(outdir / "lingam_boot_dir_prob.npy", same_dir_cond)

    print(f"[LINGAM-BOOT] Bootstraps used: {B_eff}/{args.n_boot}")
    print(f"[LINGAM-BOOT] Outputs in: {outdir}")
