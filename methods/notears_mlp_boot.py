...
LAMBDA1   = 0.01     # L1 penalty inside NOTEARS-MLP
EPS       = 1e-3    # hard cutoff on tiny weights: |W| < EPS -> 0
MAX_ITER  = 3       # keep small for speed; raise if you want better fits
...
def clean_df_no_missing(df: pd.DataFrame) -> pd.DataFrame:
    """Numeric-only; ±inf→NaN; drop NaN rows (NOTEARS-MLP needs complete data)."""
...
def main():
    ...
    n, p = X_full.shape
    print(f"[NOTEARS-MLP-BOOT] Data {X_full.shape} | n_boot={args.n_boot} | eps={EPS}, lambda1={LAMBDA1}, max_iter={MAX_ITER}")
    ...
    for b in range(args.n_boot):
        ...
        try:
            model = NotearsNonlinear(lambda1=LAMBDA1, max_iter=MAX_ITER)
            model.learn(Xb)
            W = np.asarray(model.causal_matrix, dtype=float)  # weights i->j
            np.fill_diagonal(W, 0.0)
            W[np.abs(W) < EPS] = 0.0
            A_dir = (np.abs(W) > 0).astype(int)               # 1 means i->j
        except Exception as e:
            print(f"[NOTEARS-MLP-BOOT] replicate {b}: FAILED -> {e}")
            continue
    ...
    if B_eff == 0:
        raise RuntimeError("All NOTEARS-MLP bootstraps failed; no consensus computed.")
    ...
    # ---------- save consensus PNG ----------
    png_consensus = outdir / "notears_mlp_boot_consensus.png"
    if not try_graphviz_png(consensus, labels, png_consensus):
        draw_networkx_fallback(consensus, labels, "NOTEARS-MLP bootstrap consensus", png_consensus)
    print(f"[NOTEARS-MLP-BOOT] Saved: {png_consensus.name}")
    ...
    try:
        ...
        png_annot = outdir / "notears_mlp_boot_consensus_annotated.png"
        pyd.write_png(str(png_annot))
        print(f"[NOTEARS-MLP-BOOT] Saved: {png_annot.name}")
    except Exception as e:
        print(f"[NOTEARS-MLP-BOOT] Annotation failed: {e}")

    # optional: save raw arrays
    np.save(outdir / "notears_mlp_boot_skeleton_prob.npy", skel_prob)
    np.save(outdir / "notears_mlp_boot_dir_prob.npy", same_dir_cond)

    print(f"[NOTEARS-MLP-BOOT] Bootstraps used: {B_eff}/{args.n_boot}")
    print(f"[NOTEARS-MLP-BOOT] Outputs in: {outdir}")
